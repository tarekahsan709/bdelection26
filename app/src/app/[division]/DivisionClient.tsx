...
export default function DivisionClient() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        divConstituencies.forEach((c: Constituency) =>
          districtSet.add(c.district_english),
        );
        const districts = Array.from(districtSet);
        setStats({
          totalVoters: divConstituencies.reduce(
            (sum: number, c: Constituency) => sum + c.registered_voters,
            0,
          ),
  ...
  const byDistrict = constituencies.reduce(
    (acc, c) => {
      ...
      <main className='relative z-10 max-w-6xl mx-auto px-4 py-8'>
        ...
        <section className='mb-12'>
          <DivisionMap
            ...
            className='h-64 md:h-80 rounded-2xl border border-white/[0.06]'
          />
        </section>
        ...
        <section>
          <h2 className='text-2xl md:text-3xl font-bold text-white mb-6'>
            নির্বাচনী এলাকাসমূহ
            <span className='text-neutral-500 text-lg font-normal ml-3'>
              ({stats.totalConstituencies})
            </span>
          </h2>
